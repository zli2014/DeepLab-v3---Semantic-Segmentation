...
def first_residual_block(x, kernel, out_channel, strides, is_train, name="unit"):
    with tf.variable_scope(name) as scope:
        ...
        # Shortcut connection
        shortcut = myconv(x, 1, out_channel*4, strides, name='shortcut') # 1x1 conv to obtain out_channel maps
        # Residual
        x = myconv(x, 1, out_channel, 1, name='conv_1')
        x = mygn(x, name='gn_1')
        x = myrelu(x, name='relu_1')
        x = myconv(x, kernel, out_channel, strides, name='conv_2')
        x = mygn(x, name='gn_2')
        x = myrelu(x, name='relu_2')
        x = myconv(x, 1, out_channel*4, 1, name='conv_3')
        x = mygn(x, name='gn_3')
        # Merge
        x = x + shortcut
        x = myrelu(x, name='relu_3')
    return x

def residual_block(x, kernel, is_train, name="unit"):
    num_channel = x.get_shape().as_list()[-1] / 4
    with tf.variable_scope(name) as scope:
        print('\tBuilding residual unit: %s' % scope.name)
        # Shortcut connection
        shortcut = x
        # Residual
        x = myconv(x, 1, num_channel, 1, name='conv_1')
        x = mygn(x, name='gn_1')
        x = myrelu(x, name='relu_1')
        x = myconv(x, kernel, num_channel, 1, name='conv_2')
        x = mygn(x, name='gn_2')
        x = myrelu(x, name='relu_2')
        x = myconv(x, 1, num_channel*4, 1, name='conv_3')
        x = mygn(x, name='gn_3')
        # Merge
        x = x + shortcut
        x = myrelu(x, name='relu_3')
...
def first_residual_atrous_block(x, kernel, out_channel, strides, is_train, name="unit"):
    with tf.variable_scope(name) as scope:
        print('\tBuilding residual unit: %s' % scope.name)
        # Shortcut connection
        shortcut = myatrouconv(x, 1, out_channel*4, strides, name='shortcut') # 1x1 conv to obtain out_channel maps
        # Residual
        x = myatrouconv(x, 1, out_channel, 1, name='conv_1')
        x = mygn(x, name='gn_1')
        x = myrelu(x, name='relu_1')
        x = myatrouconv(x, kernel, out_channel, strides, name='conv_2')
        x = mygn(x, name='gn_2')
        x = myrelu(x, name='relu_2')
        x = myatrouconv(x, 1, out_channel*4, 1, name='conv_3')
        x = mygn(x, name='gn_3')
        # Merge
        x = x + shortcut
        x = myrelu(x, name='relu_3')
...
def residual_atrous_block(x, kernel, strides, is_train, name="unit"):
    num_channel = x.get_shape().as_list()[-1] / 4
    with tf.variable_scope(name) as scope:
        print('\tBuilding residual unit: %s' % scope.name)
        # Shortcut connection
        shortcut = x
        # Residual
        x = myatrouconv(x, 1, num_channel, 1, name='conv_1')
        x = mygn(x, name='gn_1')
        x = myrelu(x, name='relu_1')
        x = myatrouconv(x, kernel, num_channel, strides, name='conv_2')
        x = mygn(x, name='gn_2')
        x = myrelu(x, name='relu_2')
        x = myatrouconv(x, 1, num_channel*4, 1, name='conv_3')
        x = mygn(x, name='gn_3')
        # Merge
        x = x + shortcut
        x = myrelu(x, name='relu_3')
...
def read_and_decode(data_path, epochs, batch_size):
    buffer_size = 500 * 600
    dataset = tf.data.TFRecordDataset(data_path, buffer_size=buffer_size, num_parallel_reads=32)
    dataset = dataset.apply(tf.contrib.data.shuffle_and_repeat(4000, epochs))
...
def read_and_decode_val_and_test(data_path, epochs, batch_size):
    ...
    dataset = tf.data.TFRecordDataset(data_path, buffer_size=buffer_size, num_parallel_reads=32)
    dataset = dataset.apply(tf.contrib.data.shuffle_and_repeat(4000, epochs))
    dataset = dataset.apply(tf.contrib.data.map_and_batch(parse_fn_val_and_test, batch_size, num_parallel_batches=2))
    dataset = dataset.prefetch(2)
    iterator = dataset.make_one_shot_iterator()
    batch_images, batch_labels = iterator.get_next()
    return batch_images, batch_labels
